...
// 2. 상속 + 메서드 오버라이딩
/* `Vehicle`(name)과 이를 상속하는 `Car`(name, brand).
...
// 5. 메서드 체이닝 **
/* `QueryBuilder`는 내부에 문자열을 쌓습니다.

- `select(cols)`, `from(table)`, `where(cond)`는 모두 `this`를 반환하여 체이닝.
- `build()`는 최종 문자열을 반환. */
class QueryBuilder{
  constructor(name, age, users){
    this.name = name;
    this.age = age;
    this.users = users;
  }
...
// 6. 프로토타입 확장
// `Person`(name)을 만들고, 선언 후 **프로토타입에** `greet()`를 추가하세요.
