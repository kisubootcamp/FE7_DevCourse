...
// 실행: 생성 단계에서 기록된 데이터를 바탕으로 코드를 싱행하는 일
// Outer Enviroment
...
// - 스코프 체인: 변수를 찾기 위해서 상위의 스코프로 나아가는 행위

// 변수명의 중복은 컨텍스트 기준으로 판단된다.
// 변수 쉐도잉: 같은 실행 컨텍스트를 우선으로 정의된 변수 값들 가져옴. 이때, 상위의 실행 컨텍스트의 변수가 가려진다.

//원리를 뺀 표상적인 내용
//함수 내부에서는 외부의 변수(식별자)를 참조할 수 있다.
//함수 외부에서는 내부의 변수(식별자)를 참조할 수 없다.
