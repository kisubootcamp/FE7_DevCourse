...
function mapValues(obj, func) {
	...
	keys.forEach((key) => (out[key] = func(obj[key])));
...
function invert(obj) {
	const out = {};
	const entries = Object.entries(obj); // [[ 키, 값 ]] 으로 빠지는 중첩배열
	for (const [k, v] of entries) {
		out[v] = k;
	}
	return out;
...
function getByPath(obj, str) {
	let keys = str.split("."); // ['a', 'b', 'c']
	let current = obj;

	for (let i = 0; i < keys.length; i++) {
		// if (current === null) return; // 이렇게 하면 b값을 읽어오지 못함. current가 undefined일때는 null값과 다르기때문에
		if (!current) return; // return defined     current값에 null, undefined, 0일때 무조건 거짓. !붙어서 참일때 ~
		current = current[keys[i]]; // 키값을 할당함
	}
...
// 값이 있으면 통과하고, 값이 없으면 빈객체를 생성해줘야함
const obj1 = {};
