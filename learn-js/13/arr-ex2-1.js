...
function rotateArray(arr, steps) {
	...
	// return arr.slice(-steps).concat(arr.slice(0, -steps)); // 1. 이렇게 하면 문제가 있음. steps가 arr의 길이를 넘어갈 경우
...
function maxSubArraySum(arr) {
	...
	const state = arr.slice(1).reduce(
		(s, v) => {
			s.cur = s.cur + v > v ? s.cur + v : v; // 접근한 값이랑 더했을 때 현재 나보다 커야 더함
...
// 이진 검색 ?  -> 정렬된 데이터에서 원하는 값을 찾기 위해 검색 범위를 반씩 줄여가면서 탐색하는 방법
...
function binarySearch(arr, target) {
	...
	while (left <= right) {
		...
		const half = (gap - (gap % 2)) / 2; // Math.floor((right-left) /2)
		const mid = left + half;
