...
// 1.2. 일반 함수의 this는 전역 객체를 참조한다.
// 1.2.1 자신을 호출한 객체. 함수를 어떻게, 누가 호출하냐에 따라서 this
// 1.3 화살표 함수의 this는 상위 스코프의 this를 참조한다.
...
// 3.3 자바스크립트는 이런 고전적인 클래스 기반 상속이 없던 언어 / 즉, 자바스크립트는 원래 상속이 없었음

// 3.2 프로토타입 기반의 상속 모델을 만듦
...
// 부모의 공통 메서드
Person.prototype.genInfo = function () {
  return `${this.name}은 ${this.age}세입니다.`;
};

// 자식 생성자 함수 (설계도)
function Developer(name, age, skill) {
  // 부모(Person)를 호출해서 name, age 세팅
  Person.call(this, name, age);
  this.skill = skill;
}

// 프로토타입 상속: Developer → Person
Developer.prototype = Object.create(Person.prototype);
Developer.prototype.constructor = Developer;

// Developer 인스턴스 만들기
const dev1 = new Developer("이상엽", 30, "JavaScript");

console.log(dev1.genInfo()); // "이상엽은 30세입니다."
console.log(dev1.skill); // "JavaScript"
