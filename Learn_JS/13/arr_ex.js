...
//5. 학생들의 평균 연령 구하기
console.log((students.reduce((sum, student) => sum + student.age, 0) / students.length).toFixed(1));

//6. 남학생들의 평균 연령 구하기
const maleStudents = students.filter(student => student.gender === "male");
console.log((maleStudents.reduce((sum, student) => sum + student.age, 0) / maleStudents.length).toFixed(1));

//7. 야학생들의 이름만 따로 추출해서 새로운 배열 만들기
console.log(students.filter(student => student.gender === "female").map(student => student.name));

//8. 여학생들 중 나이가 가장 어린 학생 찾기
console.log(students.filter(student => student.gender === "female").sort((a, b) => a.age - b.age)[0]);